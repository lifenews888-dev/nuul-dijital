...
export default function ContactPage() {
  ...
  useEffect(() => {
    fetch("/api/footer")
      .then((r) => r.json())
      .then((d) => {
        if (d) setContact({ ...defaultContact, ...d });
      })
      .catch(() => {});
  }, []);
  ...
  const contactInfo = [
    { icon: PhoneIcon, label: "Утас", value: contact.phone, href: `tel:${phoneClean}` },
    { icon: MailIcon, label: "Имэйл", value: contact.email, href: `mailto:${contact.email}` },
    { icon: PinIcon, label: "Хаяг", value: contact.address, href: null },
    { icon: ClockIcon, label: "Ажлын цаг", value: contact.workingHours, href: null },
  ];
