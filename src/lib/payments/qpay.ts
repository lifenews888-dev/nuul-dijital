/**
 * QPay v2 Payment Integration
 *
 * Env: QPAY_USERNAME, QPAY_PASSWORD, QPAY_INVOICE_CODE, QPAY_ENV (sandbox|production)
 */

export type QPayToken = {
  token_type: string;
...
export function getQPayCallbackUrl(): string {
  if (process.env.QPAY_CALLBACK_URL) return process.env.QPAY_CALLBACK_URL;
  const base = process.env.NEXT_PUBLIC_SITE_URL ?? "https://nuul.digital";
  return `${base.replace(/\/$/, "")}/api/payments/qpay/callback`;
}
