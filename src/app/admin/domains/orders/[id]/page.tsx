...
import { DomainOrderFulfillment } from "@/components/admin/domain-order-fulfillment";
...
export default async function AdminDomainOrderDetailPage({
  params,
}: {
  ...
  const order = await safe(
    () =>
      db.domainOrder.findUnique({
        where: { id },
        include: {
          payment: true,
          tldProduct: { select: { tld: true, labelMn: true } },
          lead: { select: { id: true } },
    ...
    <div className="max-w-5xl">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            {order.registrantVerified && <Badge variant="accent">Бичиг баталгаажсан</Badge>}
          </div>
          <p className="mt-1 text-lg font-semibold">{order.domainName}</p>
          <p className="mt-1 text-sm text-muted-foreground">
            {formatDomainPrice(order.totalAmount)} · {order.years} жил · {order.tldProduct.tld}
          </p>
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-2">
        <div className="space-y-6">
          ...
          <div className="rounded-2xl border border-white/10 bg-card p-6">
            <h2 className="mb-4 text-lg font-semibold">Бүртгэгч</h2>
            <div className="flex flex-col gap-4">
              <Field label="Төрөл">
                {REGISTRANT_TYPE_LABELS[order.registrantType] ?? order.registrantType}
              </Field>
              <Field label="Хаяг">{order.registrantAddress}</Field>
              {order.registrantType === "INDIVIDUAL" && (
                <>
                  <Field label="Бичиг баримт">
                    {order.registrantIdType
                      ? ID_TYPE_LABELS[order.registrantIdType] ?? order.registrantIdType
                      : "—"}
                  </Field>
                  <Field label="Регистрийн дугаар">{order.registrantIdNumber ?? "—"}</Field>
                </>
              )}
              {order.registrantType === "BUSINESS" && (
                <Field label="Байгууллагын регистр">{order.businessRegNumber ?? "—"}</Field>
              )}
            </div>
          </div>
        ...
        <div>
          <DomainOrderFulfillment
            order={{
              id: order.id,
              status: order.status,
              registrantVerified: order.registrantVerified,
              registrarName: order.registrarName,
              registrarOrderId: order.registrarOrderId,
              domainExpiresAt: order.domainExpiresAt,
          ...
          {(order.fulfilledAt || order.fulfilledBy) && (
            <div className="mt-6 rounded-2xl border border-white/10 bg-card p-6 text-sm text-muted-foreground">
              {order.fulfilledAt && <p>Дууссан: {formatDate(order.fulfilledAt)}</p>}
              {order.fulfilledBy && (
                <p>
                  Гүйцэтгэсэн: {order.fulfilledBy.name ?? order.fulfilledBy.email}
                </p>
              )}
            </div>
          )}
            ...
            <div className="mt-6 rounded-2xl border border-white/10 bg-card p-6 text-sm">
              ...
              {order.journey && (
                <p className="mt-1 text-muted-foreground">
                  Journey: {order.journey.currentStep}
                </p>
              )}
            </div>
          )}
